...
import dateutil
import datetime
from application.sources.tinder_api.exceptions import UserInitializationError
from application.sources.tinder_api.globals import GENDER_MAP, VALID_PHOTO_SIZES
from application.sources.tinder_api.message import Message
import six
    ...
    def __init__(self, profile_json, session):
        ...
        try:
            ...
            try:
                ...
                self.jobs.extend(
                    ["%s @ %s" % (job["title"]["name"], job["company"]["name"]) for job in profile_json['jobs'] if
                     'title' in job and 'company' in job])
                self.jobs.extend(["%s" % (job["company"]["name"],) for job in profile_json['jobs'] if
                                  'title' not in job and 'company' in job])
                self.jobs.extend(["%s" % (job["title"]["name"],) for job in profile_json['jobs'] if
                                  'title' in job and 'company' not in job])
